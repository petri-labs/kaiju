...
import (
	...
	"github.com/petri-labs/kaiju/testutil/network"
)
...
func addTestnetFlagsToCmd(cmd *cobra.Command) {
	...
	cmd.Flags().String(sdkserver.FlagMinGasPrices, fmt.Sprintf("0.000006%s", kaiju.BaseDenom), "Minimum gas prices to accept for transactions; all fees in a tx must meet this minimum (e.g. 0.01akaiju,0.001stake)")
...
Example:
	kaijud testnet init-files --validators 4 --output-dir ./.testnet --starting-ip-address 192.168.10.2
	`,
...
Example:
	kaijud testnet --validators 4 --output-dir ./.testnet
	`,
		RunE: func(cmd *cobra.Command, _ []string) error {
			args := startArgs{}
			args.outputDir, _ = cmd.Flags().GetString(flagOutputDir)
...
func initTestnetFiles(
	...
	if args.chainID == "" {
		args.chainID = fmt.Sprintf("kaiju_%d-1", tmrand.Intn(1000000))
	}
	...
	for i := 0; i < args.numValidators; i++ {
		...
		coins := sdk.Coins{
			sdk.NewCoin(kaiju.BaseDenom, accStakingTokens),
		}
		...
		createValMsg, err := stakingtypes.NewMsgCreateValidator(
			sdk.ValAddress(addr),
			valPubKeys[i],
			sdk.NewCoin(kaiju.BaseDenom, valTokens),
		...
		customAppTemplate, customAppConfig := config.AppConfig(kaiju.BaseDenom)
	...
	if err := initGenFiles(clientCtx, mbm, args.chainID, kaiju.BaseDenom, genAccounts, genBalances, genFiles, args.numValidators); err != nil {
		return err
	}
...
func initGenFiles(
	...
	govMinDepositAmt := sdk.NewIntFromBigInt(big.NewInt(0).Exp(big.NewInt(10), big.NewInt(ethermint.BaseDenomUnit), nil)).MulRaw(1) // 1 kaiju
