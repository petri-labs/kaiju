...
func (suite *KeeperTestSuite) TestVeCreate() {
	...
	testCases := []struct {
		...
		{"invalid denom", false, sender, receiver.String(), "1kaiju", types.RegulatedPeriod},
		{"invalid sender", false, []byte("xxx"), receiver.String(), denom, types.RegulatedPeriod},
		{"invalid receiver", false, sender, "xxx", "1akaiju", types.RegulatedPeriod},
...
func (suite *KeeperTestSuite) TestVeDeposit() {
	...
	testCases := []struct {
		...
		{"invalid denom", false, sender, sdk.NewCoin("kaiju", sdk.NewInt(1)), "ve-100"},
